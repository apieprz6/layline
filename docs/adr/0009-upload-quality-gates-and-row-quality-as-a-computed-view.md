...
- **The frozen-row counts above are the clip-first figures, and this decision changes them by one row.** Detecting over the whole Transcription and windowing afterwards — which is what this ADR rules — measures **870** in-window frozen rows across the thirteen recordings, not 869, and **856** of them at `SOG` of 2 knots or more, not 855. The extra row is `06-20-26-chi-wauk`'s row 56, the first in-window sample, and it is the row this section already predicted would be gained; it is also water-referenced, so the count `water_referenced` misses rises from 24 to 25 while the 845 it agrees on is unchanged. The implemented figures are pinned in `services/recordings/__tests__/archive-row-quality.test.ts`.
- **The Dropout test as implemented reads a blank as no evidence, and compares only the channels a recording feeds.** Both refine "identical latitude, longitude, `COG` and `SOG`" without weakening it, and neither changes a figure above, because all four channels are present in all 6,337 rows of the archive. A blank ends nothing: read as a change, one missing `SOG` in the middle of a latch splits the run into pieces below the minimum and reports a fabricated stretch as a clean track. And a channel the recording never fed is not compared at all, because the parser accepts an export whose header carries no `COG` — requiring the column would report every such recording dropout-free forever. Two verbatim repeats remain the positive evidence, and position is required: `SOG` alone repeating is an ordinary afternoon, so a recording with no position reports that detection was not possible rather than reporting no dropouts.
