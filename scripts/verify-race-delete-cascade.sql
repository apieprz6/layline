...
-- empties five other tables is a question for the database. So is the eighth criterion, that a
 ...
 *
 * It gets two Sail Configurations with two sails between them and one Sea State entry, so every
 * table the cascade has to reach has something in it -- including race_sail_entry_sails, which is
 * two levels down and reached only through race_sail_entries.
 */
...
BEGIN
    ...
    -- And back to deferred for the annotations: a Sail Configuration is refused for having no sails
    -- in it, which is a check that can only be true once the join rows are in -- one statement later.
    SET CONSTRAINTS ALL DEFERRED;

    -- Main and Jib 1 up at the start, from the seeded inventory.
    INSERT INTO race_sail_entries (race_id, at, reef)
    VALUES (race_id, '2026-08-22T18:30:00', 'full') RETURNING id INTO v_entry;

    INSERT INTO race_sail_entry_sails (entry_id, sail_id)
    SELECT v_entry, id FROM sails WHERE key IN ('main', 'jib-1');

    -- And a change of headsail partway through, so there is more than one entry to lose.
    INSERT INTO race_sail_entries (race_id, at, reef)
    VALUES (race_id, '2026-08-22T18:30:30', 'full') RETURNING id INTO v_entry;

    INSERT INTO race_sail_entry_sails (entry_id, sail_id)
    SELECT v_entry, id FROM sails WHERE key = 'jib-2';

    INSERT INTO race_sea_state_entries (race_id, at, sea_state)
    VALUES (race_id, '2026-08-22T18:30:00', 'moderate');

    -- Now that both Configurations have their sails, the deferred check that says so can fire.
    SET CONSTRAINTS ALL IMMEDIATE;
    SET CONSTRAINTS ALL DEFERRED;
    RESET ROLE;
    PERFORM set_config('request.jwt.claims', '', TRUE);
...
RETURNS TEXT LANGUAGE sql AS $$
    SELECT format(
        'recordings=%s rows=%s races=%s sail_entries=%s entry_sails=%s sea_state=%s',
        ...
        (SELECT count(*) FROM race_sail_entry_sails s
          WHERE EXISTS (SELECT 1 FROM race_sail_entries e
                         WHERE e.id = s.entry_id AND e.race_id = p_race_id)),
...
-- from six deletes the client sent in a row. If any of these ever became NO ACTION, the delete
...
DECLARE
    v_expected TEXT[][] := ARRAY[
        ...
        ARRAY['race_sail_entry_sails', 'entry_id', 'race_sail_entries'],
...
BEGIN
    ...
    PERFORM pg_temp.chk(
        'a sail that has been flown still cannot be deleted -- the cascade goes down, not sideways',
        (SELECT c.confdeltype FROM pg_constraint c
           JOIN pg_class child ON child.oid = c.conrelid
           JOIN pg_class parent ON parent.oid = c.confrelid
          WHERE c.contype = 'f' AND child.relname = 'race_sail_entry_sails'
            AND parent.relname = 'sails') = 'r'
    );
END;
$$;

-- ===========================================================================
-- 2. An admin's one statement takes all six tables
-- ===========================================================================
...
BEGIN
    ...
    PERFORM pg_temp.chk('the fixture is a whole race, annotations and all',
        v_before = 'recordings=1 rows=3 races=1 sail_entries=2 entry_sails=3 sea_state=1', v_before);
    ...
    PERFORM pg_temp.chk(
        'and everything that hung off it went with it -- one statement, six tables',
        pg_temp.remains(v_recording, v_race)
            = 'recordings=0 rows=0 races=0 sail_entries=0 entry_sails=0 sea_state=0',
        pg_temp.remains(v_recording, v_race)
    );
...
-- exactly one direction. The inventory, the boat and the frozen Setup Versions a race pointed at
-- are all older than the race and outlive it.
...
BEGIN
    ...
    PERFORM pg_temp.chk('the Sail Inventory is untouched by deleting a race that flew from it',
        (SELECT count(*) FROM sails) = v_sails, format('%s sails', (SELECT count(*) FROM sails)));
...
BEGIN
    ...
    PERFORM pg_temp.chk('the whole race is still there afterwards',
        pg_temp.remains(v_recording, v_race)
            = 'recordings=1 rows=3 races=1 sail_entries=2 entry_sails=3 sea_state=1',
        pg_temp.remains(v_recording, v_race));
...
BEGIN
    ...
    PERFORM pg_temp.chk(
        ...
        pg_temp.remains(v_recording, v_race)
            = 'recordings=1 rows=3 races=0 sail_entries=0 entry_sails=0 sea_state=0',
        pg_temp.remains(v_recording, v_race));
END;
$$;
...
BEGIN
    ...
    PERFORM pg_temp.chk(
        'an annotation deleted takes its own sails and nothing else -- no annotation can destroy a Transcription',
        pg_temp.remains(v_recording, v_race)
            = 'recordings=1 rows=3 races=1 sail_entries=1 entry_sails=2 sea_state=1',
        pg_temp.remains(v_recording, v_race));
END;
$$;
...
\echo ''
\echo '=== deleting a race: one statement, six tables, and the refusals ==='
