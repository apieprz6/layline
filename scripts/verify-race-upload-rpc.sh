#!/usr/bin/env bash
#
# Run the race upload transaction checks (LAY-110).
#
#   scripts/verify-race-upload-rpc.sh                 # the local stack
#   scripts/verify-race-upload-rpc.sh "$DB_URL"       # a hosted project
#   DATABASE_URL=... scripts/verify-race-upload-rpc.sh
#
...
SUITE="$HERE/verify-race-upload-rpc.sql"
...
if ! docker inspect "$CONTAINER" >/dev/null 2>&1; then
    ...
    echo "or pass a connection string: scripts/verify-race-upload-rpc.sh \"\$DB_URL\"" >&2
    exit 1
fi
